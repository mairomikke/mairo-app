...
import type { FbAttendance, FbAttendanceInsert, FbBooking, FbUser } from '@/types/firebase'

export type Attendance = FbAttendance
export type AttendanceWithUser = FbAttendance & { user: FbUser | null; booking: FbBooking | null }

const KEY = 'attendance'

// ── セッション別出席一覧（インストラクター・org用）────────────────────────────
async function fetchAttendanceForSession(sessionId: string): Promise<AttendanceWithUser[]> {
  // bookings of this session
  ...
  for (const chunk of chunks) {
    const att = await getMany<FbAttendance>('attendance', [where('booking_id', 'in', chunk)])
  ...
  const allUsers: FbUser[] = []
  for (const chunk of userChunks) {
    const users = await getMany<FbUser>('users', [where('id', 'in', chunk)])
    allUsers.push(...users)
  }
